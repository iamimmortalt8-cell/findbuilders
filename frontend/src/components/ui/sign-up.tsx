...
const modalSteps = [
    ...
    { message: "Check your email!", icon: <Mail className="w-12 h-12 text-[#7FAF8D]" /> }
];
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "FindBuilders", onSignUp, onGoogleSignIn, onNavigateToLogin, onSignUpSuccess }: AuthComponentProps) => {
  ...
  const validateField = (field: 'email' | 'password' | 'confirmPassword', val: string, passVal?: string) => {
    let err = "";
    if (field === 'email') {
      if (!val.trim()) err = "Email address is required.";
      else if (!/\S+@\S+\.\S+/.test(val)) err = "Please enter a valid email address (e.g. user@example.com).";
      ...
      else if (val !== passVal) err = "Passwords do not match.";
    }
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    const isConfValid = validateField('confirmPassword', confirmPassword, password);

    if (!isPassValid || !isConfValid) return;
  ...
  const handleProgressStep = () => {
    if (authStep === 'email') {
        ...
        if (isValid) setAuthStep("password");
    } else if (authStep === 'password') {
        setTouched(prev => ({ ...prev, password: true }));
        const isValid = validateField('password', password);
        if (isValid) setAuthStep("confirmPassword");
    }
  };
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-[#151D19]/95 border border-[#202A25] shadow-2xl rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2">
                    ...
                    {modalStatus === 'success' &&
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <p className="text-sm text-muted-foreground text-center">We sent you a confirmation email. Click the link to verify your account, then sign in.</p>
                        </div>
                    }
                </motion.div>
            </motion.div>
                        ...
                        {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-5 sm:space-y-6">
                            <BlurFade delay={authStep === 'email' ? 0.25 * 5 : 0} inView={true} className="w-full">
                                <div className="relative w-full">
                                    ...
                                    <div className={cn("glass-input-wrap w-full", fieldErrors.email && touched.email && "ring-1 ring-[#C97878]/60 rounded-full")}><div className="glass-input">
                                    ...
                                    {fieldErrors.email && touched.email && (
                                        <div className="flex items-center gap-1.5 mt-2 px-3 text-xs text-[#C97878] font-medium">
